...
# Save the settings
@spinner(text=Messages.SAVING_SETTINGS)
def decorated_save_settings(new_settings):
...
with placeholder.form(key="settings_form", clear_on_submit=False):
    ...
    new_settings["USER_EMAIL"] = st.text_input("Email", value=settings["EMAIL"])
    ...
    if submit:
        ...
        decorated_save_settings(settings)
...
if reset_settings:
    ...
    decorated_save_settings(default_settings)

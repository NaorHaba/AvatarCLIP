...
avatar_output_folder = Settings.absolute_path(os.path.join(Settings.settings['OUTPUT_DIR'], Settings.settings['GENERATED_AVATAR_OUTPUT_DIR']))
if os.path.exists(avatar_output_folder):
    ...
    if len(generated_avatars_dirs) == 0:
        ...
        if selected_avatar:
            ...
            avatar_dir_path = Settings.absolute_path(os.path.join(avatar_output_folder, selected_avatar))
            texture_folder = Settings.absolute_path(os.path.join(avatar_dir_path, Settings.settings['GENERATED_AVATAR_TEXTURE_OUTPUT_DIR']))
            fbx_file = Settings.absolute_path(os.path.join(avatar_dir_path, Settings.settings['GENERATED_AVATAR_FBX_OUTPUT_NAME']))

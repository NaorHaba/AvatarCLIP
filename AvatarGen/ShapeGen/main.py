...
def shape_gen(smpl_args, AE_path_fname, codebook_fname, neutral_txt, target_txt):
    ...
    ntxt, nweight, nstop = parse_prompt(neutral_txt)  # nweight and nstop are not used
    ttxt, tweight, tstop = parse_prompt(target_txt)  # nweight and nstop are not used
    ...
    # (vector / norm(vector)) * (vector / norm(vector)) = cos(vector, vector)
    cos_dist = (F.normalize(clip_codebook - neutral_image_embed, dim=1) * F.normalize(delta_embed.reshape(-1), dim=0)).sum(-1).reshape(-1)
...
if __name__ == '__main__':
    ...
    v, f, zero_beta_v = shape_gen(smpl_args, args.AE_path_fname, args.codebook_fname, args.neutral_txt, args.target_txt)
